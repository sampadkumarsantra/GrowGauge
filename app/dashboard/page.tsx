    ...
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-12">
      <header className="border-b border-paper-line pb-5">
        ...
        <p className="text-[13px] text-ink-mute mt-0.5">
          {session.user.role === 'facilitator' ? (
            <>
              Facilitator account · <Link href="/facilitator" className="font-semibold">Open your referral dashboard</Link>
            </>
          ) : (
      ...
      {unverified && (
        <div className="mt-6 sheet-tint px-5 sm:px-6 py-4 text-[13px] text-ink-soft leading-relaxed">
          <p>
            Your email is not verified yet. Verify it to claim scorecards created before you had an
            account and to reset your password.{' '}
            <Link href={`/login?unverified=1&email=${encodeURIComponent(session.user.email)}`} className="font-semibold">
      ...
      {submissions.length === 0 ? (
        <div className="mt-8 border border-paper-line bg-white px-5 sm:px-6 py-10 text-center">
          ...
          <p className="mt-3 text-[12px] text-ink-mute">
            ...
            <em>“Save this scorecard to my account”</em>.
          </p>
        </div>
      ) : (
